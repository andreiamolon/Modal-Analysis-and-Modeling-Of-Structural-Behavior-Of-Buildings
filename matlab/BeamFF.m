...
A= 40e-3;           Iy=133e-6;            Kv=10000000;  %Saint-Venants torsion          
ma = rho*A;         Iz=133e-6;            nelem = 4;         ndof = 30;           
% Boundary conditions
bc=[1 2 3 4 5 6 10 16 22 25 26 27 28 29 30]';
...
% 
                ...
                ep=[ E G A Iy Iz ma Kv 1.0 1.0 1.0 1.0;
		             E G A Iy Iz ma Kv 1.0 1.0 p3y p3z;
                     E G A Iy Iz ma Kv p3y p3z 1.0 1.0;
        ...
        % laboratorio para calcular os fixity factors.
                ...
                err1=sqrt((f1obj-Freq1)^2+(f2obj-Freq2)^2+(f3obj-Freq3)^2+(f4obj-Freq4)^2+(f5obj-Freq5)^2+(f6obj-Freq6)^2)/6;
                if (err1<err)
                    f1opt = Freq1; f2opt=Freq2; f3opt=Freq3; f4opt=Freq4; f5opt=Freq5; f6opt=Freq6;   
...
for i=1:3
	...
	Edb=extract(Edof,Egv(:,i));      eldisp3(Ex,Ey,Ez,Edb,[2 3 1]);
